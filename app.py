...
class SimpleSeverityAnalyzer:
    ...
    def get_severity_level(self, requirement, position, frequency, jd_text):
        score = position + frequency * 5
        if score >= 85:
            return GapSeverity.CRITICAL, "High priority requirement"
        elif score >= 60:
            return GapSeverity.HIGH, "Important requirement"
        elif score >= 35:
            return GapSeverity.MEDIUM, "Moderate requirement"
        return GapSeverity.LOW, "Optional requirement"
    ...
    def analyze_all_gaps(self, gaps_dict, jd_requirements, jd_text):
        ...
        for category, gap_list in gaps_dict.items():
            for i, gap in enumerate(gap_list):
                severity, reason = self.get_severity_level(gap, i, 1, jd_text)
                gap_item = GapItem(skill=gap, category=self.categorize_gap(gap), severity=severity, reason=reason, frequency=1, position=i)
